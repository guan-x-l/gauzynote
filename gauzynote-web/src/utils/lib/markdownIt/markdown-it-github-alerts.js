/**
 * @author https://github.com/antfu/markdown-it-github-alerts
 */
import {capitalize, getAttrsStr} from "@/utils/index.js";
...
/**
 * 自定义渲染规则处理 GitHub Alerts
 * @param md
 * @param {Object} [options] - 配置选项
 * @param {string[]} [options.markers=['TIP', 'NOTE', 'IMPORTANT', 'WARNING', 'CAUTION']] - 类型
 * @param {Object} [options.icons] - 自定义图标, 默认github svg icon
 * @param {boolean} [options.matchCaseSensitive=false] - 忽悠类型匹配时的大小写
 * @param {Object} [options.titles={}] - 自定义标题名称，需要与alert类型对应
 * @param {string} [options.classPrefix='markdown-alert'] - 类名前缀
 */
export default function githubAlertsPlugin(md, options = {}) {
    const {
    ...
    md.renderer.rules.alert_open = function (tokens, idx) {
        const { title, type, icon, map } = tokens[idx].meta
        const attrsStr = getAttrsStr(tokens[idx].attrs)
        return `<div class="${classPrefix} ${classPrefix}-${type}" ${attrsStr}><p class="${classPrefix}-title">${icon}${title}</p>`
    }
}